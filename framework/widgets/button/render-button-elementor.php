...
if ( ! empty( $atts['link_source'] ) && isset( $dynamic_content ) && ! empty( $dynamic_content['source'] ) ) {
    $btn_link = apply_filters( 'alpus_dynamic_tags_content', '', null, $dynamic_content );
...
if ( $self ) { // For Elementor
    $label = alpus_widget_button_get_label( $atts, $self, $label, 'label' );
} else { // For Gutenberg
    if ( $label ) {
...
if ( isset( $play_btn ) && $play_btn ) {
    $alpus_section['video_btn'] = true;
    $class .= ' btn-video elementor-custom-embed-image-overlay';
    $options                    = array();

    if ( isset( $alpus_section['lightbox'] ) ) {
        $options = $alpus_section['lightbox'];
    }
    echo '<div class="' . esc_attr( $class ) . '" role="button"' . ( $options ? ( ' data-elementor-open-lightbox="yes" data-elementor-lightbox="' . esc_attr( wp_json_encode( $options ) ) . '"' ) : '' ) . '>' . alpus_strip_script_tags( $label ) . '</div>';
} elseif ( $video_btn ) {
    wp_enqueue_style( 'alpus-lightbox' );
    wp_enqueue_script( 'alpus-lightbox' );

    $class .= ' btn-video-iframe';
    printf( '<a class="' . esc_attr( $class ) . '" href="' . esc_url( ! empty( $video_url['url'] ) ? $video_url['url'] : '#' ) . '">%1$s</a>', alpus_strip_script_tags( $label ) );
} else {
    $attrs = array();

    if ( $self ) {
        $attrs['href']   = ! empty( $link['url'] ) ? alpus_strip_script_tags( $link['url'] ) : '#';
    ...
    if ( $self ) { //For Elementor
        printf( '<a class="' . esc_attr( $class ) . '" ' . $link_attrs . '>%1$s</a>', alpus_strip_script_tags( $label ) );
    } else { //For Gutenberg
        if ( $button_text_hover_effect ) {
            $class .= ' btn-text-hover-effect ' . $button_text_hover_effect;
        }
        $link = $link ? $link : '#';
        printf( '<div class="btn-wrapper ' . esc_attr( $wrap_class ) . '"><a class="' . esc_attr( $class ) . '" ' . 'href="' . $link . '">%1$s</a></div>', alpus_strip_script_tags( $gb_label ) );
    }
}
