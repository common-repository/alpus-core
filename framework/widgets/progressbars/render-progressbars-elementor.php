...
if ( count( $progressbars_list ) ) {
    ...
    foreach ( $progressbars_list as $key => $bar ) {
        ...
        if ( 'outer' == $text_pos ) {
            ...
            echo alpus_escaped( $title_html );

            if ( 'yes' == $display_percentage ) {
                echo alpus_escaped( $percent_html ) . '</div>';
            }
        }

        $progress_class = 'progress-wrapper';

        echo '<div class="' . esc_attr( $progress_class ) . '" data-value="' . floatval( $percent ) . '"><div class="progress-bar bg-' . alpus_escaped( $bar['progress_skin'] ) . '">';
        ...
        if ( 'yes' == $display_percentage && 'inner' == $text_pos ) {
            echo alpus_escaped( $percent_html );
        }
        echo '</div></div>';
    }
    echo '</div>';
