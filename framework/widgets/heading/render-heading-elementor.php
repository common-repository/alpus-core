...
$link_label = '<span ' . ( $self ? $self->get_render_attribute_string( 'link_label' ) : '' ) . '>' . alpus_strip_script_tags( $link_label ) . '</span>';
...
if ( $title ) {
    ...
    if ( $self ) {
        $html .= sprintf( '<%1$s ' . $self->get_render_attribute_string( 'title' ) . '>%2$s</%1$s>', esc_html( $tag ), alpus_strip_script_tags( $title ) );
    } else {
        $heading_link = '';
