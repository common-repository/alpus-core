...
extract( // @codingStandardsIgnoreLine
    shortcode_atts(
        array(
            ...
            'role'                => esc_html__( 'Customer', 'alpus-core' ),
            'link'                => '',
            'comment_title'       => '',
...
if ( defined( 'ELEMENTOR_VERSION' ) ) {
    ...
    if ( is_numeric( $avatar ) ) {
        ...
        $img_alt  = $img_alt ? esc_attr( trim( $img_alt ) ) : esc_attr__( 'Testimonial Image', 'alpus-core' );
...
if ( $rating && 'simple' != $testimonial_type ) {
    ...
    $rating_html .= '<div class="ratings-container"><div class="ratings-full' . esc_attr( $rating_cls ) . '" style="letter-spacing: ' . $rating_sp['size'] . 'px;" aria-label="' . esc_attr( sprintf( __( 'Rated %s out of 5', 'alpus-core' ), $rating ) ) . '"><span class="ratings" style="width: ' . $rating_w . '; letter-spacing: ' . $rating_sp['size'] . 'px;"></span></div></div>';
}
$commenter = '<cite><span class="name">' . esc_html( $name ) . '</span><span class="role">' . esc_html( $role ) . '</span></cite>';

if ( ! empty( $testimonial_type ) ) {
    require alpus_core_framework_path( ALPUS_CORE_FRAMEWORK_PATH . "/widgets/testimonial/templates/render-{$testimonial_type}.php" );
}
