...
/**
 * Alpus Posts Grid Widget Render
 *
 * @author     D-THEMES
 *
 * @since      1.2.0
...
if ( 'archive' == $shortcode_type ) {
    ...
    if ( 'product' == $post_type ) {
        ...
        if ( isset( $_COOKIE[ ALPUS_NAME . '_gridcookie' ] ) && 'list' == sanitize_text_field( $_COOKIE[ ALPUS_NAME . '_gridcookie' ] ) && ! empty( $list_builder_id ) ) {
...
if ( empty( $source ) && 'yes' == $filter_cat && $filter_cat_tax ) {
    ...
    if ( count( $filter_terms ) > 1 ) {
        ...
        if ( $category_html ) {
            ...
            $category_html = '<ul class="nav-filters post-filters">' . ( 'yes' == $show_all_filter ? '<li class="nav-filter-clean"><a href="' . esc_url( $all_link ) . '" class="nav-filter' . ( ! $active_cat ? ' active' : '' ) . '" data-cat="*">' . esc_html__( 'All', 'alpus-core' ) . '</a></li>' : '' ) . $category_html . '</ul>';
    ...
    if ( 'terms' == $source ) { // terms
        ...
        if ( 'product' == $post_type && class_exists( 'WooCommerce' ) ) {
            ...
            if ( 'viewed' == $product_status ) {
                $viewed_products = ! empty( $_COOKIE['woocommerce_recently_viewed'] ) ? (array) explode( '|', sanitize_text_field( wp_unslash( $_COOKIE['woocommerce_recently_viewed'] ) ) ) : array(); // @codingStandardsIgnoreLine
...
if ( $should_render_wrapper ) {
    ...
    if ( empty( $source ) && $is_filter_cat ) {
        wp_enqueue_style( 'alpus-tab', alpus_core_framework_uri( '/widgets/tab/tab' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ), array(), ALPUS_CORE_VERSION );
    }

    $wrapper_id      = 'alpus-posts-grid-' . rand( 1000, 9999 );
    $wrap_class      = 'alpus-posts-grid';
    $container_class = 'alpus-posts-grid-container';
    $container_attrs = '';

    // grid / list toggle classes
    if ( ! empty( $toggle_cols ) ) {
        $container_attrs .= ' data-toggle_cls="' . esc_attr( trim( alpus_get_col_class( alpus_elementor_grid_col_cnt( $toggle_cols ) ) ) ) . '"';
    }
    ...
    if ( $builder_post ) {
        $css  = get_post_meta( $builder_id, ALPUS_NAME . '_blocks_style_options_css', true );
        $css .= get_post_meta( $builder_id, '_' . ALPUS_NAME . '_builder_css', true );
        $css .= get_post_meta( $builder_id, 'page_css', true );

        if ( $list_builder_id ) {
            $css .= get_post_meta( $list_builder_id, ALPUS_NAME . '_blocks_style_options_css', true );
            $css .= get_post_meta( $list_builder_id, '_' . ALPUS_NAME . '_builder_css', true );
        ...
        if ( $css ) {
            if ( alpus_is_elementor_preview() ) {
                ...
                wp_add_inline_style( 'alpus-style', '/* Post Type Builder CSS */' . PHP_EOL . wp_strip_all_tags( $css ) );
            }
        }
        ...
        if ( $page_js ) {
            if ( alpus_is_elementor_preview() ) {
                echo '<script>';
                echo alpus_strip_script_tags( $page_js );
                echo '</script>';
            } else {
                wp_add_inline_script( 'alpus-theme', '/* Post Type Builder JS */' . PHP_EOL . alpus_strip_script_tags( $page_js ) );
            }
        }
    }
    ...
    if ( 'creative' == $view ) {
        ...
        $container_class .= ' ' . alpus_get_slider_class( $atts );
        ...
        $container_attrs .= " data-creative-breaks='" . wp_json_encode(
            array(
                'sm'  => alpus_get_breakpoints( 'sm' ),
                'md'  => alpus_get_breakpoints( 'md' ),
                'lg'  => alpus_get_breakpoints( 'lg' ),
                'xl'  => alpus_get_breakpoints( 'xl' ),
                'xlg' => alpus_get_breakpoints( 'xlg' ),
                'xxl' => alpus_get_breakpoints( 'xxl' ),
            )
        ) . "'";
        $container_attrs .= ' data-grid-options="' . esc_attr(
            wp_json_encode(
                array(
                    'itemSelector' => '.alpus-tb-item',
                    'layoutMode'   => 'masonry',
    ...
    if ( $thumbnail_size ) {
        $GLOBALS['alpus_post_image_size'] = $thumbnail_size;
    }

    if ( ! empty( $category_html ) ) {
        echo apply_filters( 'alpus_products_filter_cat_html', $category_html );
    }

    // Load More Properties
    if ( empty( $source ) ) {
        $atts['shortcode']       = 'alpus-posts-grid';
    ...
    if ( $builder_post && current_user_can( 'edit_pages' ) && ! is_customize_preview() &&
        ...
        $builder_type = get_post_meta( $builder_id, ALPUS_NAME . '_template_type', true );
    ...
    if ( $edit_link && $builder_post ) {
        ...
        $edit_link_html = '<div class="alpus-edit-link d-none" data-title="' . sprintf( esc_attr__( 'Edit %1$s: %2$s', 'alpus-core' ), esc_attr( str_replace( '_', ' ', $builder_type ) ), esc_attr( get_the_title( $builder_id ) ) ) . '" data-link="' . esc_url( $edit_link ) . '"></div>';
    }
    /*
    ...
    if ( 'slider' == $view && $edit_link_html ) {
        echo alpus_strip_script_tags( $edit_link_html );
    }
    ...
    if ( isset( $posts_query ) && ! $posts_query->have_posts() && ! empty( $post_found_nothing ) && 'archive' == $shortcode_type ) {
        echo '<div class="nothing-found-message w-100">' . alpus_strip_script_tags( $post_found_nothing ) . '</div>';
    }
    ...
    if ( empty( $source ) && $posts_query->have_posts() ) { // posts
        ...
        if ( 'product' == $post_type ) {
            if ( function_exists( 'alpus_get_option' ) && alpus_get_option( 'compare_available' ) ) {
                wp_enqueue_style( 'alpus-product-compare', alpus_core_framework_uri( '/addons/product-compare/product-compare' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ), array(), ALPUS_VERSION );
                wp_enqueue_script( 'alpus-product-compare', alpus_core_framework_uri( '/addons/product-compare/product-compare' . ALPUS_JS_SUFFIX ), array( 'alpus-framework-async' ), ALPUS_VERSION, true );
            }

            if ( function_exists( 'alpus_quickview_add_scripts' ) ) {
                alpus_quickview_add_scripts();
            }
            ...
            if ( defined( 'ALPUS_VERSION' ) ) {
                wp_enqueue_style( 'alpus-theme-single-product', ALPUS_ASSETS . '/css/pages/single-product' . ( is_rtl() ? '-rtl' : '' ) . '.min.css', array(), ALPUS_VERSION );
            }
            wp_enqueue_script( 'alpus-woocommerce' );

            if ( isset( $GLOBALS['product'] ) ) {
                $original_product = $GLOBALS['product'];
            }

            $GLOBALS['alpus_tb_catalog_mode'] = false;

            if ( function_exists( 'alpus_get_option' ) && alpus_get_option( 'catalog_mode' ) ) {
                $GLOBALS['alpus_tb_catalog_mode'] = true;
            }

            if ( ! $builder_post ) {
                wc_set_loop_prop( 'alpus_print_content_only', true );
            }
            ...
            if ( 'archive' == $shortcode_type ) {
                $show_info      = alpus_wc_category_show_info();
                ...
                if ( $categories_html ) {
                    wp_enqueue_style( 'alpus-product-category', alpus_core_framework_uri( '/widgets/categories/category' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ), array(), ALPUS_CORE_VERSION );
                    echo apply_filters( 'alpus_posts_grid_product_subcategories_html', str_replace( array( '<li class="category-wrap', '</li>' ), array( '<div class="category-wrap', '</div>' ), $categories_html ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                }
        ...
        while ( $posts_query->have_posts() ) {
            ...
            if ( 'creative' == $view && isset( $post_count ) ) {
                $post_count++;
                $repeaters = alpus_get_loop_prop( 'repeaters' );
                $item_cls .= ' grid-item';
                ...
                if ( ! empty( $repeaters ) ) {
                    ...
                    if ( isset( $repeaters['images'][ $post_count ] ) ) {
                        $GLOBALS['alpus_post_image_size'] = $repeaters['images'][ $post_count ];
                    }
                }
             ...
             * @since 1.0
             */
            do_action( 'alpus_post_loop_before_item' );
            ...
            if ( $builder_post ) {
                ...
                if ( 'product' == $post_type ) {
                    ...
                    alpus_get_template_part( 'posts/type/post', 'default' );
                }
            }
             ...
             * @since 1.0
             */
            do_action( 'alpus_post_loop_after_item' );

            do_action( 'alpus_posts_grid_item_rendered', $builder_post, $atts );
            echo '</div>';
        }
        ...
        $alpus_page_layout = alpus_get_page_layout();
        ...
        foreach ( $posts as $term ) {
            ...
            echo '<div class="' . esc_attr( $item_cls ) . '"' . alpus_escaped( $item_attrs ) . '>';
            ...
            if ( $builder_post ) {
                ...
                alpus_get_template_part( 'posts/type/category', 'default' );
            }
        ...
        $alpus_page_layout = '';
        unset( $GLOBALS['alpus_page_layout'] );
    }
    $GLOBALS['wp_query']                 = $original_query; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
    $GLOBALS['wp_query']->queried_object = $original_queried_object; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited

    unset( $GLOBALS['alpus_post_image_size'], $GLOBALS['woocommerce_loop'] );

    // set posts layout to grid to display pagination
    if ( 'creative' == $view || 'masonry' == $view ) {
        alpus_set_loop_prop( 'posts_layout', 'grid' );
    }
