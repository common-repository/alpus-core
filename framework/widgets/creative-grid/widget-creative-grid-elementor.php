...
/*
 * Alpus Creative Grid Widget Addon
 *
 * Alpus Creative Grid Widget Addon using Elementor Section/Column Element
 *
 * @author     D-THEMES
...
if ( ! class_exists( 'Alpus_Creative_Grid_Elementor_Widget_Addon' ) ) {
    class Alpus_Creative_Grid_Elementor_Widget_Addon extends Alpus_Base {
        ...
        public function __construct() {
            ...
            add_action( 'alpus_elementor_column_addon_content_template', array( $this, 'column_addon_content_template' ), 1 );
        ...
        public function register_section_addon( $addons ) {
            $addons['creative'] = esc_html__( 'Creative', 'alpus-core' );
        ...
        public function add_section_controls( $self, $condition_value ) {
            $self->add_control(
                ...
                array(
                    'raw'             => sprintf( esc_html__( 'Use %1$schild columns%2$s as %1$sgrid item%2$s by using %1$s%3$s settings%2$s.', 'alpus-core' ), '<b>', '</b>', ALPUS_DISPLAY_NAME ),
            ...
            $self->start_controls_section(
                ...
                array(
                    'label'     => alpus_elementor_panel_heading( esc_html__( 'Creative Grid', 'alpus-core' ) ),
            ...
            alpus_elementor_creative_layout_controls( $self, $condition_value, 'section' );
        ...
        public function section_addon_attributes( $options, $self, $settings ) {
            if ( 'creative' == $settings['use_as'] ) {
                ...
                $alpus_section = array(
                    'section' => 'creative',
                    'preset'  => alpus_creative_layout( $settings['creative_mode'] ),
        ...
        public function section_addon_render( $self, $settings ) {
            if ( 'creative' == $settings['use_as'] ) {
                ...
                if ( 'yes' == $settings['grid_float'] ) {
                    ...
                    $extra_attrs .= " data-creative-breaks='" . wp_json_encode(
                        array(
                ...
                alpus_creative_layout_style(
                    '.elementor-element-' . $self->get_data( 'id' ),
                    $alpus_section['layout'],
				...
				<div class="<?php echo esc_attr( 'yes' == $settings['section_content_type'] ? 'elementor-container container-fluid' : 'elementor-container' ); ?> elementor-column-gap-no">
					<!-- Start .elementor-row -->
					<div class="elementor-row<?php echo esc_attr( $extra_class ); ?>"<?php echo alpus_escaped( $extra_attrs ); ?>>
				<?php
        ...
        public function add_column_controls( $self, $condition_value ) {
            $self->add_responsive_control(
                'creative_width',
                array(
                    'label'       => esc_html__( 'Grid Item Width (%)', 'alpus-core' ),
                    'type'        => Controls_Manager::NUMBER,
                    'description' => esc_html__( 'This Option will be applied when only parent section is used for creative grid. Empty Value will be set from preset of parent creative grid.', 'alpus-core' ),
                    'min'         => 1,
                    'max'         => 100,
                ),
                array(
            ...
            $self->add_responsive_control(
                'creative_height',
                array(
                    'label'       => esc_html__( 'Grid Item Height', 'alpus-core' ),
                    ...
                    'options'     => array(
                        ...
                        '1-4'    => '1/4',
                        '3-4'    => '3/4',
                        'child'  => esc_html__( 'Depending on Children', 'alpus-core' ),
                        'preset' => esc_html__( 'Use From Preset', 'alpus-core' ),
                    ),
                    'label_block' => 'true',
                    'description' => esc_html__( 'This Option will be applied when only parent section is used for creative grid.', 'alpus-core' ),
                ),
                array(
                    'position' => array(
                        'at' => 'before',
            ...
            $self->add_responsive_control(
                'creative_order',
                array(
                    'label'       => esc_html__( 'Grid Item Order', 'alpus-core' ),
                    'type'        => Controls_Manager::SELECT,
                    'separator'   => 'after',
                    'default'     => '1',
                    'options'     => array(
                        '1'  => esc_html__( '1', 'alpus-core' ),
                        '2'  => '2',
                        '3'  => '3',
                        '4'  => '4',
                        '5'  => '5',
                    ...
                    'description' => esc_html__( 'This Option will be applied when only parent section is used for creative grid not float grid.', 'alpus-core' ),
                ),
                array(
                    'position' => array(
                        'at' => 'before',
        ...
        public function column_addon_attributes( $options, $self, $settings ) {
            ...
            if ( isset( $alpus_section['section'] ) && 'creative' == $alpus_section['section'] && $alpus_section['top'] == $self->get_data( 'isInner' ) ) {
                $idx       = $alpus_section['index'];
                $classes[] = 'grid-item';
                $grid      = array();

                if ( $idx < count( $alpus_section['preset'] ) ) {
                    foreach ( $alpus_section['preset'][ $idx ] as $key => $value ) {
                        if ( 'h' == $key ) {
                            continue;
                        }
                ...
                if ( isset( $settings['creative_height'] ) && 'preset' == $settings['creative_height'] ) {
                    $grid['h'] = $idx < count( $alpus_section['preset'] ) ? $alpus_section['preset'][ $idx ]['h'] : '1-3';
 ...
 * @since 1.2.0
 */
Alpus_Creative_Grid_Elementor_Widget_Addon::get_instance();
