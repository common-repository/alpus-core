<?php
/**
 * Alpus Image Gallery Widget Render
 *
 * @author     D-THEMES
 *
 * @since      1.0
...
$extra_class .= alpus_get_grid_space_class( $atts );
...
if ( 'creative' == $layout_type ) {
    ...
    $extra_class .= ' ' . alpus_get_grid_space_class( $atts );
    $extra_class .= ' ' . alpus_get_slider_class( $atts );
...
<ul class="<?php echo esc_attr( $extra_class ); ?>"<?php echo alpus_escaped( $extra_attrs ); ?>>
	<?php
    foreach ( $images as $index => $attachment ) {
        ...
        echo '<li class="' . esc_attr( $img_wrap_class ) . '"' . alpus_escaped( $wrap_attrs ) . '>';
        ?>
		<div class="image-gallery-item<?php echo ( ! $overlay || $overlay_wrapper ) ? '' : ' ' . esc_attr( alpus_get_overlay_class( $overlay ) ); ?>">
		<?php
        ...
        if ( $overlay_wrapper ) {
            ...
            echo '<div class="overlay-effect overlay-' . alpus_escaped( $overlay ) . '"></div>';
        } else {
            if ( 'yes' == $image_popup ) {
                echo '<a href="' . esc_url( ! empty( $full_src[0] ) ? $full_src[0] : '' ) . '"></a>';
            }
        ...
        if ( ! $image_html && ! empty( $attachment['url'] ) ) {
            echo '<img src="' . esc_url( $attachment['url'] ) . '" alt="' . esc_html__( 'Image Gallery Item', 'alpus-core' ) . '">';
        } else {
            echo alpus_escaped( $image_html );
        }
        ...
        if ( ! empty( $image_caption ) ) {
            echo '<figcaption class="elementor-image-carousel-caption">' . alpus_strip_script_tags( $image_caption ) . '</figcaption>';
        }
        ?>
		</figure>
		</div>
