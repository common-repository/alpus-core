...
class Alpus_Admin_Meta_Boxes extends Alpus_Base {
    ...
    public function __construct() {
        if ( is_admin() ) {
            ...
            if ( ! class_exists( 'MB_Tabs' ) ) {
                require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/meta-box-tabs/meta-box-tabs.php' );
                // require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/mb-rest-api/mb-rest-api.php' );
                // require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/mb-settings-page/mb-settings-page.php' );
                // require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/mb-term-meta/mb-term-meta.php' );
                require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/meta-box-columns/meta-box-columns.php' );
                // require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/meta-box-conditional-logic/meta-box-conditional-logic.php' );
                // require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/meta-box-group/meta-box-group.php' );
                // require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/meta-box-include-exclude/meta-box-include-exclude.php' );
                // require_once alpus_core_framework_path( ALPUS_CORE_PLUGINS . '/meta-box/extensions/meta-box-show-hide/meta-box-show-hide.php' );
            }
        ...
        add_filter( 'alpus_dynamic_tags', array( $this, 'metabox_add_tags' ) );
        add_filter( 'rwmb_meta_boxes', array( $this, 'get_meta_boxes' ), 99999 );
        add_filter( 'alpus_dynamic_extra_fields_content', array( $this, 'metabox_render' ), 10, 3 );
        add_filter( 'alpus_dynamic_field_object', array( $this, 'metabox_source' ) );
        add_action( 'alpus_dynamic_extra_fields', array( $this, 'metabox_add_control' ), 10, 3 );
    ...
    public function add_meta_box( $meta_boxes ) {
        ...
        if ( alpus_is_elementor_preview() && ALPUS_NAME . '_template' == $post_type && ( 'single' == get_post_meta( (int) $_GET['post'], ALPUS_NAME . '_template_type', true ) || 'archive' == get_post_meta( (int) $_GET['post'], ALPUS_NAME . '_template_type', true ) ) ) {
            do_action( 'alpus_core_dynamic_before_render', $post_type, (int) $_GET['post'] );
        ...
        $meta_tabs = array(
            'titles'  => array(
                'label' => __( 'Page Titles', 'alpus-core' ),
                'icon'  => 'dashicons-heading',
            ),
            'scripts' => array(
                'label' => __( 'Custom Scripts', 'alpus-core' ),
        ...
        $meta_fields = array(
            ...
            'page_subtitle' => array(
                'id'      => 'page_subtitle',
                'name'    => __( 'Page Subtitle', 'alpus-core' ),
                'desc'    => '',
                'type'    => 'text',
                'std'     => '',
                'columns' => 12,
        ...
        if ( 'post' == $post_type ) {
            $meta_tabs['post'] = array(
                'label'    => __( 'Post Options', 'alpus-core' ),
                'icon'     => 'dashicons-admin-post',
                'priority' => 5,
            );
        }

        if ( 'post' == $post_type || ALPUS_NAME . '_template' == $post_type ) {
            $meta_fields['supported_images'] = array(
                'id'                => 'supported_images',
                'type'              => 'file_advanced',
                'name'              => esc_html__( 'Supported Images', 'alpus-core' ),
                'save_field'        => true,
                'label_description' => esc_html__( 'These images will be shown as slider with Featured Image.', 'alpus-core' ),
                'tab'               => 'post',
            );
            $meta_fields['featured_video']   = array(
                'id'                => 'featured_video',
                'type'              => 'textarea',
                'name'              => esc_html__( 'Featured Video', 'alpus-core' ),
                'save_field'        => true,
                'label_description' => esc_html__( 'Input embed code or use shortcodes. ex) iframe-tag or', 'alpus-core' ) . ' [video src="url.mp4"]',
                'tab'               => 'post',
            );
        }
         ...
         * @since 1.0
         */
        $meta_tabs = apply_filters( 'alpus_metabox_tabs', $meta_tabs, $post_type );
        /**
         * Filters metabox fields.
         *
         * @since 1.0
         */
        $meta_fields = apply_filters( 'alpus_metabox_fields', $meta_fields, $post_type );

        uasort( $meta_tabs, array( $this, 'sort_priority' ) );
        usort( $meta_fields, array( $this, 'sort_priority' ) );

        $meta_boxes[] = array(
            'title'      => sprintf( esc_html__( '%s Options', 'alpus-core' ), ALPUS_DISPLAY_NAME ),
        ...
        if ( alpus_is_elementor_preview() && ALPUS_NAME . '_template' == $post_type && ( 'single' == get_post_meta( (int) $_GET['post'], ALPUS_NAME . '_template_type', true ) || 'archive' == get_post_meta( (int) $_GET['post'], ALPUS_NAME . '_template_type', true ) ) ) {
            do_action( 'alpus_core_dynamic_after_render', $post_type, (int) $_GET['post'] );
        }
    ...
    public function add_product_cat_fields( $tag ) {
        if ( is_object( $tag ) ) { ?>
			<tr class="form-field">
				<th scope="row"><label for="product_cat_icon"><?php esc_html_e( 'Category Icon', 'alpus-core' ); ?></label></th>
				<td>
					<input name="product_cat_icon" id="product_cat_icon" type="text" value="<?php echo esc_attr( get_term_meta( $tag->term_id, 'product_cat_icon', true ) ); ?>" placeholder="<?php esc_attr_e( 'Input icon class here...', 'alpus-core' ); ?>">
				</td>
			</tr>
		<?php } else { ?>
			<div class="form-field">
				<label for="product_cat_icon"><?php esc_html_e( 'Category Icon', 'alpus-core' ); ?></label>
				<input name="product_cat_icon" id="product_cat_icon" type="text" placeholder="<?php esc_attr_e( 'Input icon class here...', 'alpus-core' ); ?>">
			</div>
			<?php
		}
    }
    ...
    public function save_term_meta_box( $term_id, $tt_id, $taxonomy ) {
        if ( 'product_cat' == $taxonomy ) {
            if ( isset( $_POST['product_cat_icon'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
                update_term_meta( $term_id, 'product_cat_icon', sanitize_text_field( $_POST['product_cat_icon'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            } else {
    ...
    public function metabox_add_tags( $tags ) {
        array_push( $tags, 'Alpus_Core_Custom_Field_Meta_Box_Tag', 'Alpus_Core_Custom_Image_Meta_Box_Tag', 'Alpus_Core_Custom_Gallery_Tag' );
    ...
    public function metabox_source( $objects ) {
        $objects['meta-box'] = esc_html__( 'Meta Box', 'alpus-core' );
        ...
        if ( 'meta-box' == $plugin ) {
            ...
            if ( 'image' == $widget ) {
                $object->add_control(
                    'add_featured_image',
                    array(
                        'label' => esc_html__( 'Add Featured Image', 'alpus-core' ),
            ...
            $object->add_control(
                $control_key,
                array(
                    'label'   => esc_html__( 'MetaBox Field', 'alpus-core' ),
    ...
    public function metabox_fields( $group = 'field' ) {
        ...
        foreach ( $groups as $data ) {
            if ( ALPUS_NAME . '_template' != $type && false === in_array( $type, $data['post_types'] ) ) {
                continue;
            }
    ...
    public function metabox_get_meta( $key ) {
        ...
        if ( 'page_title' == $key || 'page_subtitle' == $key ) {
            ...
            if ( 'page_title' == $key ) {
                ...
                if ( ! $meta_value ) {
                    $meta_value = array( $alpus_layout['title'] );
                }
                ...
                if ( ! $meta_value ) {
                    $meta_value = array( $alpus_layout['subtitle'] );
                }
            }
        } else {
...
Alpus_Admin_Meta_Boxes::get_instance();
