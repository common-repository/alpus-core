<?php
/**
 * Alpus Dynamic Tags Content class
 *
 * @author     D-THEMES
 *
 * @since      1.2.0
...
if ( ! class_exists( 'Alpus_Gutenberg_Dynamic_Tags_Content' ) ) {
    class Alpus_Gutenberg_Dynamic_Tags_Content extends Alpus_Base {
        ...
        public function __construct() {
            add_filter( 'alpus_dynamic_tags_content', array( $this, 'get_dynamic_content_result' ), 10, 5 );

            add_action( 'wp_ajax_alpus_dynamic_tags_get_value', array( $this, 'get_value' ) );
            add_action( 'wp_ajax_alpus_dynamic_tags_acf_fields', array( $this, 'get_acf_fields' ) );
        }
        ...
        public function get_dynamic_content_result( $default = false, $object = null, $settings = array(), $data_type = '', $image_size = 'full' ) {
            ...
            if ( ! $data && ! empty( $settings['fallback'] ) ) {
                $data = alpus_strip_script_tags( $settings['fallback'] );
            ...
            if ( ! empty( $settings['before'] ) ) {
                $data = alpus_strip_script_tags( $settings['before'] ) . $data;
            }

            if ( ! empty( $settings['after'] ) ) {
                $data .= alpus_strip_script_tags( $settings['after'] );
            }
        ...
        public function get_value() {
            check_ajax_referer( 'alpus-core-nonce', 'nonce' );

            if ( isset( $_POST['content_type'] ) && isset( $_POST['content_type_value'] ) && ! empty( $_POST['source'] ) && ! empty( $_POST['field_name'] ) ) {
                $atts   = array(
                    'content_type'       => sanitize_text_field( $_POST['content_type'] ),
                    'content_type_value' => sanitize_text_field( $_POST['content_type_value'] ),
                );
                $object = apply_filters( 'alpus_builder_get_current_object', false, $atts );

                if ( $object ) {
                    if ( 'term' == $atts['content_type'] && 'post' == sanitize_text_field( $_POST['source'] ) ) {
                    } elseif ( 'term' != $atts['content_type'] && $atts['content_type'] && 'tax' == sanitize_text_field( $_POST['source'] ) ) {
                    } else {
                        $result = $this->get_dynamic_content( false, $object, sanitize_text_field( $_POST['source'] ), sanitize_text_field( $_POST['field_name'] ) );

                        if ( false === $result ) {
                            wp_send_json_error();
                        }

                        if ( isset( $_POST['type'] ) && 'image' == sanitize_text_field( $_POST['type'] ) ) {
                            $image_size = isset( $_POST['img_size'] ) ? sanitize_text_field( $_POST['img_size'] ) : 'full';
        ...
        public function get_acf_fields() {
            ...
            if ( class_exists( 'ACF' ) && isset( $_POST['content_type'] ) && isset( $_POST['content_type_value'] ) ) {
                ...
                if ( $object ) {
                    $fields = apply_filters( 'alpus_gutenberg_editor_vars', array(), $object );
...
Alpus_Gutenberg_Dynamic_Tags_Content::get_instance();
