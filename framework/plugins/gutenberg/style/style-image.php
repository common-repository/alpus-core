...
function alpus_get_filter_style( $style ) {
    $html                         = '';
...
if ( ! empty( $settings['img_align_selector'] ) ) {
    echo alpus_escaped( $settings['selector'] . '{ text-align: ' . esc_html( $settings['img_align_selector'] ) . '; }' );
}

if ( ! empty( $settings['img_filter_selector'] ) ) {
    echo alpus_escaped( $settings['selector'] . ' img {' . alpus_get_filter_style( $settings['img_filter_selector'] ) . '}' );
}

if ( ! empty( $settings['img_hover_filter_selector'] ) ) {
    echo alpus_escaped( $settings['selector'] . ':hover img {' . alpus_get_filter_style( $settings['img_hover_filter_selector'] ) . '}' );
}
...
if ( ! empty( $settings['show_caption_selector'] ) && '' != $settings['show_caption_selector'] && ! empty( $settings['caption_style_selector'] ) ) {
    echo alpus_escaped( $settings['selector'] . ' figcaption {' );

    if ( ! empty( $settings['caption_style_selector']['textAlign'] ) ) {
        echo 'text-align:' . esc_html( $settings['caption_style_selector']['textAlign'] ) . ';';
    }

    if ( ! empty( $settings['caption_style_selector']['backgroundColor'] ) ) {
        echo 'background-color:' . esc_html( $settings['caption_style_selector']['backgroundColor'] ) . ';';
    }

    if ( ! empty( $settings['caption_style_selector']['marginTop'] ) ) {
        echo 'margin-top:' . esc_html( $settings['caption_style_selector']['marginTop'] ) . 'px;';
    }
    echo '}';
}
