 ...
 */
defined( 'ABSPATH' ) || die;
define( 'ALPUS_CORE_GUTENBERG', ALPUS_CORE_FRAMEWORK_PATH . '/plugins/gutenberg' );

class Alpus_Gutenberg extends Alpus_Base {

    /**
         * Gutenberg widget names.
         *
         * @var array WP Alpus gutenberg widget names
         *
         * @since 1.2.0
         */
    ...
    public function __construct() {
        /*
         * Gutenberg filter name to add alpus css class to the widgets
         *
         * @since 1.2.0
         */
        ...
        $this->gutenberg_blocks = apply_filters( 'alpus_gutenberg_blocks', $this->gutenberg_blocks );

        // Init gutenberg editor
        add_action( 'current_screen', array( $this, 'current_screen' ) );
        $this->add_gutenberg_blocks();
        // Dynamic Tags
        require_once alpus_core_framework_path( ALPUS_CORE_GUTENBERG . '/dynamic_tags/dynamic-tags.php' );
    }
    ...
    public function enqueue_block_editor_assets_style() {
        wp_enqueue_style( 'alpus-blocks-style-editor', alpus_core_framework_uri( '/plugins/gutenberg/assets/css/gutenberg-editor' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ), array( 'alpus-admin' ), ALPUS_CORE_VERSION );
        wp_enqueue_script( 'alpus-blocks', alpus_core_framework_uri( '/plugins/gutenberg/assets/gutenberg.min.js' ), array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-data', 'alpus-admin' ), ALPUS_CORE_VERSION, true );

        if ( defined( 'ALPUS_VERSION' ) ) {
            alpus_load_google_font();
            Alpus_Layout_Builder::get_instance()->setup_layout();
            ob_start();
            include alpus_core_framework_path( ALPUS_CORE_GUTENBERG . '/gutenberg-variable.php' );
            $output_style = ob_get_clean();

            if ( function_exists( 'alpus_minify_css' ) ) {
                $output_style = alpus_minify_css( $output_style );
            }

            wp_add_inline_style( 'alpus-blocks-style-editor', $output_style );
        }
        $js_alpus_block_vars['googlefonts'] = class_exists( 'Kirki_Fonts' ) ? Kirki_Fonts::get_google_fonts() : array();
        $js_alpus_block_vars['theme_url']   = esc_url( get_parent_theme_file_uri() );
        $js_alpus_block_vars['core_url']    = ALPUS_CORE_FRAMEWORK_URI;

        if ( is_admin() && get_current_screen()->is_block_editor() ) {
            $js_alpus_block_vars['edit_post_id'] = (int) get_the_ID();
        }

        $image_sizes = array();

        foreach ( alpus_get_image_sizes() as $value => $key ) {
            $image_sizes[] = array(
                'label' => str_replace( '&amp;', '&', esc_js( $value ) ),
                'value' => esc_js( $key ),
            );
        }
        $js_alpus_block_vars['image_sizes'] = $image_sizes;

        wp_localize_script(
            'alpus-blocks',
            'alpus_block_vars',
            apply_filters( 'alpus_gutenberg_editor_vars', $js_alpus_block_vars )
        );
    }
    ...
    public function save_meta_values( $post_id, $post ) {
        ...
        // save dynamic styles
        if ( false !== strpos( $post->post_content, '<!-- wp:' . ALPUS_NAME ) ) { // Gutenberg editor
            ...
            if ( ! empty( $blocks ) ) {
                ...
                if ( $css ) {
                    update_post_meta( $post_id, ALPUS_NAME . '_blocks_style_options_css', wp_strip_all_tags( $css ) );
                } else {
                    delete_post_meta( $post_id, ALPUS_NAME . '_blocks_style_options_css' );
                }
            }
        }
    }
    ...
    protected function include_style( $blocks, $saved_css = '' ) {
        ...
        foreach ( $blocks as $block ) {
            if ( ! empty( $block['blockName'] ) && 0 === strpos( $block['blockName'], ALPUS_NAME ) ) {
                $prefix     = ALPUS_NAME . '/' . ALPUS_NAME . '-';
                $block_name = '';
                ...
                foreach ( $block['attrs'] as $key => $value ) {
                    if ( false !== strpos( $key, 'font_settings' ) || false !== strpos( $key, 'style_options' ) || false !== strpos( $key, '_selector' ) || 0 === strpos( $key, 'st_' ) || ( 'spacing' == $key && ALPUS_NAME . '-tb/' . ALPUS_NAME . '-meta' == $block['blockName'] ) || ( 0 === strpos( $key, 'hover_' ) && ALPUS_NAME . '-tb/' . ALPUS_NAME . '-featured-image' == $block['blockName'] ) ) {
                        $attrs[ $key ] = $value;
                    }
                }
                ...
                if ( ! empty( $attrs ) ) {
                    $selector = '.alpus-gb-' . self::get_global_hashcode( $attrs, str_replace( ALPUS_NAME . '/' . ALPUS_NAME . '-', '', $block['blockName'] ) );
                    $settings = $attrs;

                    $settings['selector'] = $selector;

                    if ( ALPUS_NAME . '/' . ALPUS_NAME . '-heading' === $block['blockName'] ) {
                        $settings['selector'] .= ' .title';
                    } elseif ( ALPUS_NAME . '/' . ALPUS_NAME . '-button' === $block['blockName'] ) {
                        $settings['selector'] .= ' .btn';
                    }

                    // Type Builder Blocks
                    if ( ALPUS_NAME . '-tb/' . ALPUS_NAME . '-woo-price' == $block['blockName'] ) {
                        $settings['selector'] .= ' .price';
                    } elseif ( ALPUS_NAME . '-tb/' . ALPUS_NAME . '-woo-rating' == $block['blockName'] ) {
                    ...
                    } elseif ( ALPUS_NAME . '-tb/' . ALPUS_NAME . '-woo-stock' == $block['blockName'] ) {
                        $settings['selector'] .= ' .stock';
                    } elseif ( ALPUS_NAME . '-tb/' . ALPUS_NAME . '-woo-desc' == $block['blockName'] ) {
                        $settings['selector'] .= ' p';
                    }
                    ...
                    if ( $block_name ) {
                        ob_start();
                        include alpus_core_framework_path( ALPUS_CORE_GUTENBERG . '/style/style-' . $block_name . '.php' );
                    ...
                    $part_css = apply_filters( 'alpus_gutenberg_block_style', '', $block['blockName'], $block['attrs'], $selector );
                    ...
                    foreach ( $attrs as $key => $value ) {
                        ...
                        if ( false !== strpos( $key, 'font_settings' ) ) {
                            if ( ALPUS_NAME . '/' . ALPUS_NAME . '-icon-box' === $block['blockName'] ) {
                            ...
                            ob_start();
                            include alpus_core_framework_path( ALPUS_CORE_GUTENBERG . '/style/style-font.php' );
                            $part_css = ob_get_clean();
                            ...
                            ob_start();
                            include alpus_core_framework_path( ALPUS_CORE_GUTENBERG . '/style/style-options.php' );
                            $part_css = ob_get_clean();
        ...
        foreach ( $this->gutenberg_blocks as $block => $attr ) {
            $callback = function ( $atts, $content ) use ( $block ) {
                if ( ! empty( $atts ) && ! empty( $block ) ) {
                    ...
                    $selector = 'alpus-gb-' . self::get_global_hashcode( $atts_for_hash, $block );
                }
                ...
                ob_start();
                include alpus_core_framework_path( ALPUS_CORE_FRAMEWORK_PATH . "/widgets/{$block}/render-{$block}-elementor.php" );
            ...
            if ( $is_gutenberg ) {
                register_block_type(
                    ALPUS_NAME . '/' . ALPUS_NAME . '-' . $block,
                    array(
                        'attributes'      => isset( $attr ) ? $attr : array(),
                        'editor_script'   => 'alpus-blocks',
