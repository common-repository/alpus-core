 ...
 * @since 1.2.1
 */
if ( ! class_exists( 'Alpus_Conditional_Rendering' ) ) {
    class Alpus_Conditional_Rendering extends Alpus_Base {
        ...
        public function add_condition_system( $self ) {
            $self->start_controls_section(
                'section_conditional',
                array(
                    'label' => alpus_elementor_panel_heading( esc_html__( 'Conditional Rendering System', 'alpus-core' ) ),
            ...
            $repeater->add_control(
                'condition_a',
                array(
                    'label'       => esc_html__( 'Condition A', 'alpus-core' ),
                    'description' => esc_html__( 'Select condition type.', 'alpus-core' ),
                    'type'        => Controls_Manager::SELECT,
                    'options'     => array(
                        'device'       => esc_html__( 'Device', 'alpus-core' ),
                        'login_status' => esc_html__( 'Login Status', 'alpus-core' ),
                        'user_role'    => esc_html__( 'User Role', 'alpus-core' ),
                        'post_page'    => esc_html__( 'Post & Page', 'alpus-core' ),
                    ),
                )
            );
            $repeater->add_control(
                ...
                array(
                    'label'   => esc_html__( 'Comparative Operator', 'alpus-core' ),
                    'type'    => Controls_Manager::SELECT,
                    'options' => array(
                        'equal'     => esc_html__( '==', 'alpus-core' ),
                        'not_equal' => esc_html__( '!=', 'alpus-core' ),
                    ),
                )
            );
            $repeater->add_control(
                'value_device',
                array(
                    'label'     => esc_html__( 'Device', 'alpus-core' ),
                    'type'      => Controls_Manager::SELECT,
                    'options'   => array(
                        'desktop'       => esc_html__( 'Desktop', 'alpus-core' ),
                        'tablet_mobile' => esc_html__( 'Tablet & Mobile', 'alpus-core' ),
            ...
            $repeater->add_control(
                'value_login',
                array(
                    'label'     => esc_html__( 'Status', 'alpus-core' ),
                    'type'      => Controls_Manager::SELECT,
                    'options'   => array(
                        'login'  => esc_html__( 'Logged In', 'alpus-core' ),
                        'logout' => esc_html__( 'Logged Out', 'alpus-core' ),
                    ),
                    'condition' => array(
            ...
            $repeater->add_control(
                'condition_operator',
                array(
                    'label'       => esc_html__( 'Operator', 'alpus-core' ),
                    'description' => esc_html__( 'The selected value is used to operate on the conditions below.', 'alpus-core' ),
                    'type'        => Controls_Manager::SELECT,
                    'options'     => array(
                        'and' => esc_html__( 'And', 'alpus-core' ),
                        'or'  => esc_html__( 'Or', 'alpus-core' ),
                    ),
                )
            );
            ...
            $self->add_control(
                ...
                array(
                    ...
                    'raw'             => esc_html__( 'Only when these conditions are matched, will this section be rendered.', 'alpus-core' ),
            ...
            $self->add_control(
                'conditional_render',
                array(
                    'label'         => esc_html__( 'Conditional Render', 'alpus-core' ),
        ...
        public function should_render( $should_render, $self ) {
            ...
            if ( function_exists( 'alpus_is_elementor_preview' ) && ! alpus_is_elementor_preview() && ! $this->is_render( $atts ) ) {
                return false;
            }
...
Alpus_Conditional_Rendering::get_instance();
