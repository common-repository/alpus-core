...
add_action( 'elementor/frontend/column/before_render', 'alpus_column_render_attributes', 10, 1 );

if ( ! class_exists( 'Alpus_Element_Column' ) ) {
    class Alpus_Element_Column extends Elementor\Element_Column {
        ...
        protected function register_controls() {
             ...
             * @since 1.0
             */
            $column_addons = apply_filters( 'alpus_elementor_column_addons', array() );

            if ( ! empty( $column_addons ) ) {
                $column_addons = array_merge( array( '' => esc_html__( 'Default', 'alpus-core' ) ), $column_addons );

                $this->add_control(
                    'use_as',
                    array(
                        'type'    => Controls_Manager::SELECT,
                        'label'   => esc_html__( 'Use Column For', 'alpus-core' ),
                        'default' => '',
            ...
            $this->add_control(
                ...
                array(
                    ...
                    'label'       => esc_html__( 'Sticky Column', 'alpus-core' ),
                    'default'     => 'no',
                    'separator'   => 'before',
                    'description' => esc_html__( 'Controls column to make sticky or not in a section.', 'alpus-core' ),
                )
            );
            ...
            $this->add_responsive_control(
                ...
                array(
                    ...
                    'label'     => esc_html__( 'Top Space on Sticky', 'alpus-core' ),
                    'default'   => '0',
                    'selectors' => array(
                        '{{WRAPPER}}.elementor-column' . ( alpus_elementor_if_dom_optimization() ? ' > .elementor-widget-wrap' : ' > .elementor-column-wrap' ) => 'top: {{SIZE}}px',
                    ),
                    'condition' => array(
                        'sticky_column' => 'yes',
                    ),
             ...
             * @since 1.0
             */
            do_action( 'alpus_elementor_column_addon_controls', $this, 'use_as' );
            alpus_elementor_addon_controls( $this, 'column' );
        }
        ...
        protected function content_template() {
			...
			<#
                 ...
                 * @since 1.0
                 */
                do_action( 'alpus_elementor_column_addon_content_template', $this );
            ?>
				...
				<# } else { #>
					...
					<div class="elementor-<?php echo esc_attr( $wrapper_element ); ?>-wrap" {{{ wrapper_attrs }}}>
        ...
        public function before_render() {
            ...
            $this->add_render_attribute(
                array(
                    '_wrapper'            => array(
                        'class' => ( 'yes' == $settings['sticky_column'] ) ? 'alpus-sticky-column' : '',
                    ),
             ...
             * @since 1.0
             */
            do_action( 'alpus_elementor_column_render', $this, $settings, $has_background_overlay, $is_legacy_mode_active );
            $addon_html = ob_get_clean();

            if ( ! $addon_html ) {
                ?>
				<<?php echo alpus_escaped( $this->get_html_tag() ) . ' ' . $this->get_render_attribute_string( '_wrapper' ); ?>>
        ...
        public function after_render() {
             ...
             * @since 1.0
             */
            do_action( 'alpus_elementor_column_after_render', $this, $settings, $is_legacy_mode_active );
            $addon_html = ob_get_clean();
...
if ( ! function_exists( 'alpus_column_render_attributes' ) ) {
    /**
     * Add render attributes for columns.
     *
     * @since 1.0
     */
    function alpus_column_render_attributes( $self ) {
        ...
        $options = apply_filters(
            'alpus_elementor_column_addon_render_attributes',
            array(
        ...
        $self->add_render_attribute(
            array(
                '_wrapper'        => alpus_get_elementor_addon_options( $settings, $options['wrapper_args'] ),
                '_inner_wrapper'  => $options['inner_args'],
                '_widget_wrapper' => $options['widget_args'],
            )
        );

        if ( $settings['background_image'] && $settings['background_image']['url'] && function_exists( 'alpus_get_option' ) && alpus_get_option( 'lazyload' ) ) {
            if ( ! is_admin() && ! is_customize_preview() && ! alpus_doing_ajax() ) {
                $data = array(
                    'data-lazy' => esc_url( $settings['background_image']['url'] ),
                );

                if ( ! $settings['background_color'] ) {
                    $data['style'] = 'background-color: ' . alpus_get_option( 'lazyload_bg' ) . ';';
                }
