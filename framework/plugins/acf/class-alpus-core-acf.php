<?php
/**
 * Alpus ACF plugin compatibility for dynamic tags.
 *
 * @author     D-THEMES
 *
 * @version    1.0
...
class Alpus_Core_Acf extends Alpus_Base {
    ...
    public function __construct() {
        add_filter( 'alpus_dynamic_tags', array( $this, 'acf_add_tags' ) );
        add_filter( 'alpus_dynamic_field_object', array( $this, 'acf_add_object' ) );
        add_filter( 'alpus_dynamic_extra_fields_content', array( $this, 'acf_render' ), 10, 3 );
        add_action( 'alpus_dynamic_extra_fields', array( $this, 'acf_add_control' ), 10, 3 );

        add_filter( 'alpus_gutenberg_editor_vars', array( $this, 'retrieve_acf_meta_fields' ), 10, 2 );
    }

    /**
     * Returns Alpus support acf types
     *
     * @return array
     */
    ...
    public function acf_add_tags( $tags ) {
        array_push( $tags, 'Alpus_Core_Custom_Field_Acf_Tag', 'Alpus_Core_Custom_Image_Acf_Tag' );
    ...
    public function acf_add_object( $objects ) {
        $objects['acf'] = esc_html__( 'ACF', 'alpus-core' );
    ...
    public function acf_add_control( $object, $widget = 'field', $plugin = 'acf' ) {
        if ( 'acf' == $plugin ) {
            ...
            $object->add_control(
                $control_key,
                array(
                    'label'   => esc_html__( 'ACF Field', 'alpus-core' ),
    ...
    public function get_acf_groups( $widget, $object = null ) {
        ...
        if ( function_exists( 'acf_get_field_groups' ) ) {
            ...
            } elseif ( $post && ALPUS_NAME . '_template' == get_post_type( $post ) && 'type' == get_post_meta( $post->ID, ALPUS_NAME . '_template_type', true ) ) {
