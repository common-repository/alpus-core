...
if ( ! class_exists( 'Alpus_Product_Ordering' ) ) {
    /**
     * Alpus Product Ordering Class
     *
     * @since 1.0
     */
    class Alpus_Product_Ordering {
        ...
        public function __construct() {
             ...
             * @since 1.0
             */
            do_action( 'alpus_after_product_ordering', $this );
        }
        ...
        public function order_popularity_date( $args ) {
            if ( false !== strpos( $args['where'], "post_type = 'product'" ) ) {
                ...
                if ( $this->order_to ) {
                    $where .= $wpdb->prepare(
                        ( $where ? ' AND ' : '' ) . ' date_created <= %s', // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                        $this->order_to
                    );
                }
             ...
             * @since 1.0
             */
            return apply_filters( 'alpus_wc_order_popularity_date', $args );
        }
        ...
        public function order_wish_count( $args ) {
             ...
             */
            return apply_filters( 'alpus_wc_order_wish_count', $args );
        }
        ...
        public function order_sale_end_date( $args ) {
             ...
             * @since 1.0
             */
            return apply_filters( 'alpus_wc_order_sale_end_date', $args );
        }
        ...
        public function order_sale_start_date( $args ) {
             ...
             * @since 1.0
             */
            return apply_filters( 'alpus_wc_order_sale_start_date', $args );
        }
        ...
        public function order_created_date( $args ) {
             ...
             * @since 1.0
             */
            return apply_filters( 'alpus_wc_order_create_date', $args );
        }
        ...
        protected function parse_date() {
            if ( $this->order_from ) {
                if ( 'today' == $this->order_from ) {
                    $this->order_from = gmdate( 'Y-m-d', strtotime( 'today' ) );
                } elseif ( 'week' == $this->order_from ) {
                    $this->order_from = gmdate( 'Y-m-d', strtotime( 'this Monday' ) );
                } elseif ( 'month' == $this->order_from ) {
                    $this->order_from = gmdate( 'Y-m-01' );
                } elseif ( 'year' == $this->order_from ) {
                    $this->order_from = gmdate( 'Y-01-01' );
                }
            }

            if ( $this->order_to ) {
                if ( 'today' == $this->order_to ) {
                    $this->order_to = gmdate( 'Y-m-d', strtotime( 'tomorrow' ) );
                } elseif ( 'week' == $this->order_to ) {
                    $this->order_to = gmdate( 'Y-m-d', strtotime( 'this Sunday' ) );
                } elseif ( 'month' == $this->order_to ) {
                    $this->order_to = gmdate( 'Y-m-01', mktime( 0, 0, 0, gmdate( 'm' ) + 1, gmdate( 'd' ), gmdate( 'Y' ) ) );
                } elseif ( 'year' == $this->order_to ) {
                    $this->order_to = gmdate( 'Y-01-01', mktime( 0, 0, 0, gmdate( 'm' ), gmdate( 'd' ), gmdate( 'Y' ) + 1 ) );
                }
            }
        }
