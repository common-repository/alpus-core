...
if ( ! class_exists( 'Alpus_Breadcrumb' ) ) {
    /**
     * Breadcrumb class.
     */
    class Alpus_Breadcrumb extends Alpus_Base {
        ...
        public function __construct() {
            ...
            add_action( 'alpus_breadcrumb', array( $this, 'print' ) );
        }
        ...
        private function set_args( $atts ) {
            ...
            if ( ! empty( $atts ) ) { // breadcrumb widget option
                ...
                if ( isset( $atts['delimiter_icon'] ) && is_array( $atts['delimiter_icon'] ) && $atts['delimiter_icon']['value'] ) {
                    ...
                    $delimiter = alpus_strip_script_tags( $atts['delimiter'] );
                } else {
                    $delimiter = '/';
                }

                if ( 'yes' == $atts['home_icon'] ) {
                    $args['home'] = '<i class="' . ALPUS_ICON_PREFIX . '-icon-home"></i>';
                    $extra_class .= ' home-icon';
                }
            } else { // theme option
                $delimiter = alpus_strip_script_tags( alpus_get_option( 'ptb_delimiter' ) );

                if ( ! $delimiter ) {
                    $delimiter = '<i class="' . ALPUS_ICON_PREFIX . '-icon-angle-right"></i>';
                }
            }
             ...
             * @since 1.0
             */
            $this->args = apply_filters( 'alpus_breadcrumb_args', $args );
        }
        ...
        private function get_breadcrumbs() {
            ...
            // Home Link
            if ( ! is_front_page() ) {
                $this->breadcrumbs[ esc_html__( 'Home', 'alpus-core' ) ] = apply_filters( 'woocommerce_breadcrumb_home_url', home_url() );
            }
            ...
            if ( is_singular() ) {
                 ...
                 * @since 1.0
                 */
                if ( apply_filters( 'alpus_is_vendor_store', false ) ) {
                    ...
                    if ( is_tag() ) {
                        ...
                        $title                       = sprintf( esc_html__( 'Tag - %s', 'alpus-core' ), $title );
                        $this->breadcrumbs[ $title ] = '';
                    } elseif ( is_tax( 'product_tag' ) ) {
                        $title                       = sprintf( esc_html__( 'Product Tag - %s', 'alpus-core' ), $title );
                        $this->breadcrumbs[ $title ] = '';
                    } else {
                        $this->breadcrumbs[ $title ] = '';
                    }
            ...
            $this->breadcrumbs = apply_filters( 'alpus_breadcrumb_items', $this->breadcrumbs );
        }
        ...
        private function _get_archive_title_link() {
             ...
             * @since 1.0
             */
            $link = apply_filters( 'alpus_get_archive_link', get_post_type_archive_link( $post_type ), $post_type );
        ...
        private function _get_archive_name( $post_type ) {
             ...
             * @since 1.0
             */
            return apply_filters( 'alpus_get_archive_name', $archive_title, $post_type );
        }
        ...
        private function _get_leaf_title( $object_type = '' ) {
            ...
            switch ( $object_type ) {
                ...
                case 'search':
                    ...
                    $title = sprintf( esc_html__( 'Search - %s', 'alpus-core' ), $search );
                    break;

                case '404':
                    $title = esc_html__( '404', 'alpus-core' );
                    break;
                default:
        ...
        private function print_breadcrumbs() {
            ...
            echo alpus_strip_script_tags( $output );
        }
        ...
        private function get_structured_data() {
            ...
            foreach ( $this->breadcrumbs as $title => $link ) {
                ...
                if ( ! empty( $link ) ) {
                    ...
                    $current_url = set_url_scheme( sanitize_url( 'http://' . wp_unslash( $_SERVER['HTTP_HOST'] ) . wp_unslash( $_SERVER['REQUEST_URI'] ) ) );
            ...
            if ( preg_match( '|^[a-zA-Z]{1,20}$|', $microdata['@type'] ) ) {
                 ...
                 * @since 1.0
                 */
                $this->structured_data = apply_filters( 'alpus_breadcrumb_structured_data', $microdata, $this->breadcrumbs );
            } else {
        ...
        private function print_structured_data() {
            ...
            echo '<script ' . apply_filters( 'alpus_script_tag', false ) . '>' . _wp_specialchars( wp_json_encode( $this->structured_data ), ENT_NOQUOTES ) . '</script>';
        }
    }
}

Alpus_Breadcrumb::get_instance();
