...
foreach ( $args['blocks'] as $block ) {
    if ( $block instanceof WP_Post ) {
		...
		<div class="<?php echo esc_attr( $class ); ?>">
			<img src="<?php echo esc_url( ALPUS_SERVER_URI . ( 'wpalpus' == ALPUS_NAME ? 'framework' : ALPUS_NAME ) . '/dummy/images/studio/' . intval( isset( $block['s'] ) ? $block['s'] : $block['block_id'] ) . '.jpg' ); ?>" alt="<?php echo esc_attr( $block['t'] ); ?>"<?php echo isset( $block['w'] ) && $block['w'] ? ' width="' . intval( $block['w'] ) . '"' : '', isset( $block['h'] ) && $block['h'] ? ' height="' . intval( $block['h'] ) . '"' : ''; ?>>
			...
			<div class="block-actions" data-id="<?php echo esc_attr( $block['block_id'] ); ?>" data-category="<?php echo esc_attr( $block['c'] ); ?>">
				...
				<?php if ( class_exists( 'Alpus_Admin' ) && Alpus_Admin::get_instance()->is_registered() ) { ?>
					<button class="btn <?php echo boolval( $args['studio']->new_template_mode ) ? 'select' : 'import'; ?>">
						<i class="fas fa-download"></i>
					</button>
				<?php } ?>
