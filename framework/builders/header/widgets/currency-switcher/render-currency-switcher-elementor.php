...
if ( has_nav_menu( 'cur-switcher' ) ) {
    ...
    foreach ( $currencies as $currency ) {
        ...
        if ( $currency['name'] == $current_currency ) {
            ...
            $other_c .= '<li rel="' . esc_attr( $currency['name'] ) . '"><a href="#">' . alpus_strip_script_tags( $label ) . '</a></li>';
        }
    }
    ?>
	<ul class="menu switcher cur-switcher">
		<a href="#" class="switcher-toggle"><?php echo alpus_strip_script_tags( $active_c ); ?></a>
		<?php if ( $other_c ) { ?>
		<ul>
			<?php echo alpus_strip_script_tags( $other_c ); ?>
		</ul>
		<?php } ?>
	</ul>
	<?php
    ...
    if ( $woocommerce_wpml->multi_currency ) {
         ...
         * @since 1.0
         */
        $format        = apply_filters( 'alpus_wcml_multi_currency_format', '%symbol% %code%' );
        ...
        foreach ( $currencies as $currency ) {
            if ( 1 == $woocommerce_wpml->settings['currency_options'][ $currency ]['languages'][ $sitepress->get_current_language() ] ) {
                ...
                if ( $selected ) {
                    ...
                    $other_c .= '<li><a href="#" rel="' . esc_attr( $currency ) . '">' . alpus_strip_script_tags( $currency_format ) . '</a></li>';
                }
		...
		<ul class="menu switcher cur-switcher wcml_currency_switcher" id="menu-currency-switcher">
			<li class="menu-item-has-children">
				<a class="switcher-toggle" href="#"><?php echo alpus_strip_script_tags( $active_c ); ?></a>
				<?php if ( $other_c ) { ?>
	...
	<ul class="menu switcher cur-switcher">
		<li class="menu-item-has-children">
			<a class="switcher-toggle" href="#"><?php esc_html_e( 'USD', 'alpus-core' ); ?></a>
			<ul>
				<li><a href="#"><?php esc_html_e( 'EUR', 'alpus-core' ); ?></a></li>
				<li><a href="#"><?php esc_html_e( 'USD', 'alpus-core' ); ?></a></li>
			</ul>
		</li>
	</ul>
	<?php
