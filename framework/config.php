<?php
/**
 * The base configuration for Alpus Core FrameWork
 *
...
/*
 * Defines Alpus Core FrameWork Version
 * Defines Alpus Core FrameWork Path
 */
define( 'ALPUS_CORE_FRAMEWORK_VERSION', '1.3.0' );
define( 'ALPUS_CORE_FRAMEWORK_PATH', ALPUS_CORE_PATH . '/framework' );
define( 'ALPUS_CORE_FRAMEWORK_URI', ALPUS_CORE_URI . '/framework' );
 ...
 */
if ( ! function_exists( 'alpus_core_framework_path' ) ) {
 ...
 */
if ( ! function_exists( 'alpus_core_framework_uri' ) ) {
 ...
 *                      ...
 */
if ( ! function_exists( 'alpus_add_feature' ) ) {
    function alpus_add_feature( $features ) {
        if ( empty( $features ) ) {
            return false;
        }
 ...
 * framework doesn't require feature related file.
 *
 * @see alpus_add_feature()
 * @param array|string $features Features for framework.
 */
if ( ! function_exists( 'alpus_remove_feature' ) ) {
    function alpus_remove_feature( $features ) {
        if ( empty( $features ) ) {
            return false;
        }
 ...
 */
if ( ! function_exists( 'alpus_get_feature' ) ) {
    function alpus_get_feature( $feature ) {
        if ( ! empty( $feature ) ) {
...
/*
 * Alpus FrameWork Setup Configuration
 *
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpus_setup' ) ) {
    function alpus_setup() {
        $addon_features   = apply_filters(
            'alpus_addon_features',
            array(
                'fs_addon_walker',
                'fs_addon_skeleton',
         ...
         * @since 1.0
         */
        $extra_features     = apply_filters( 'alpus_extra_features', array() );
        ...
        alpus_add_feature( $framework_features );
    }
}

alpus_setup();
/*
 * Fires after setup Alpus Core FrameWork configuration.
 *
 * @since 1.0
 */
do_action( 'alpus_after_core_framework_config' );
