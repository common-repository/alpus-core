...
extract( // @codingStandardsIgnoreLine
    shortcode_atts(
        array(
            ...
            'account_dropdown' => '',
            'account_avatar'   => '',
            'icon'             => ALPUS_ICON_PREFIX . '-icon-account',
        ),
        $atts
    )
);
...
if ( is_user_logged_in() ) {
    ...
    $html .= '<a class="login logout ' . esc_attr( $type ) . '" href="' . esc_url( $logout_link ) . '" aria-label="' . esc_attr__( 'Logout', 'alpus-core' ) . '">';
    ...
    if ( in_array( 'login', $items ) ) {
        ...
        $html .= '<span>' . alpus_strip_script_tags( str_replace( '%name%', $user ? $user->display_name : '', $logout_text ) ) . '</span>';
    }
    $html .= '</a>';
        ...
        if ( ! has_nav_menu( 'account-menu' ) ) {
            ...
            foreach ( wc_get_account_menu_items() as $endpoint => $label ) {
                if ( 'wishlist' == $endpoint ) {
                     ...
                     * @since 1.0
                     */
                    $url = apply_filters( 'alpus_account_dashboard_link', '' );
                } else {
                ...
                $html .= '<a href="' . esc_url( $url ) . '" aria-label="' . esc_attr__( 'Dashboard', 'alpus-core' ) . '">' . esc_html( $label ) . '</a>';
                $html .= '</li>';
            }
            $html .= '</ul>';
            $html .= '</div>';
    ...
    if ( in_array( 'login', $items ) || ( ! in_array( 'register', $items ) && in_array( 'icon', $items ) ) ) {
        $html .= '<a class="login ' . ( 'offcanvas' !== $form_type ? esc_attr( $form_type ) : '' ) . ' ' . esc_attr( $type ) . '" href="' . esc_url( $login_link ) . '" aria-label="' . esc_attr__( 'Login', 'alpus-core' ) . '">';
    ...
    if ( in_array( 'register', $items ) ) {
        ...
        $html .= '<a class="register ' . ( 'offcanvas' !== $form_type ? esc_attr( $form_type ) : '' ) . ' ' . esc_attr( $type ) . '" href="' . ( $register_link ? esc_url( $register_link ) : esc_url( $login_link ) ) . '" aria-label="' . esc_attr__( 'Register', 'alpus-core' ) . '">';
...
echo '<div class="account ' . esc_attr( $type ) . ' ' . esc_attr( $extra_class ) . '">' . alpus_escaped( $html ) . '</div>';
