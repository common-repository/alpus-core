	...
	function setupStudioBlocks() {
				...
				$.ajax( {
					...
					success: function( response ) {
						...
						if ( searchPos == 'widget-search' ) {
							...
							if ( $candidateBlocks.length && $( '.demo-filter input[type=search]' ).val() == filterVal ) {
								$candidateBlocks.append( newItems.clone() );
							}
						}
		...
		function importBlockHandler( e ) {
			...
			themeAdmin.prompt.showDialog({
				title: alpus_studio.pro_title,
				content: alpus_studio.pro_desc,
				actions: [
					{
					callback: function() {
						window.open(alpus_studio.purchase_pro);
					}
					},
					{ callback: "" }
				]
		...
		if ( $( '.blocks-wrapper' ).length ) {
			$( '#elementor-panel' ).on( 'mousewheel', '#elementor-panel-content-wrapper', function() {
				var $candidateBlocks = $( '#alpus-studio-candidate-blocks' );
				if ( $candidateBlocks.length && $( '.demo-filter input[type=search]' ).val() ) {
			...
			// 	$( '.demo-filter input[type=search]' ).val( $this.val() );
