    ...
    if ( empty( $source ) && $posts_query->have_posts() ) { // posts
        ...
        if ( 'product' == $post_type ) {
            if ( defined( 'ALPUS_PRODUCT_COMPARE_VERSION' ) ) {
                ...
                wp_enqueue_script( 'alpus-product-compare', ALPUS_PRODUCT_COMPARE_ASSETS . 'js/product-compare' . ALPUS_JS_SUFFIX, array( 'alpus-framework-async' ), ALPUS_PRODUCT_COMPARE_VERSION, true );
            }
        ...
        while ( $posts_query->have_posts() ) {
            ...
            if ( $edit_link && $builder_post ) {
                ...
                echo '<div class="alpus-edit-link d-none" data-title="' . sprintf( esc_attr__( 'Edit %1$s: %2$s', 'alpus-core' ), esc_attr( str_replace( '_', ' ', $builder_type ) ), esc_attr( get_the_title( $builder_id ) ) ) . '" data-link="' . esc_url( $edit_link ) . '"></div>';
