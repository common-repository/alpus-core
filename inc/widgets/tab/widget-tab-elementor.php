...
/*
 * Alpus Tab Widget Addon
 *
 * Alpus Tab Widget Addon using Elementor Section/Column Element
 *
 * @author     AlpusTheme
 * @package    Alpus Core
 * @subpackage Core
...
class Alpus_Tab_Elementor_Widget_Addon extends Alpus_Base {
    ...
    public function register_section_addon( $addons ) {
        $addons['tab'] = esc_html__( 'Tab', 'alpus-core' );
    ...
    public function add_section_controls( $self, $condition_value ) {
        $self->add_control(
            ...
            array(
                'description' => sprintf( esc_html__( '%1$s%2$sNote:%3$s Use %2$schild columns%3$s as %2$stab content%3$s by using %2$s%4$s%3$s settings%5$s', 'alpus-core' ), '<span class="important-note">', '<b>', '</b>', ALPUS_DISPLAY_NAME, '</span>' ),
                'type'        => 'description',
                'condition'   => array(
                    $condition_value => 'tab',
                ),
        ...
        alpus_elementor_tab_layout_controls( $self, $condition_value );

        $self->end_controls_section();

        alpus_elementor_tab_style_controls( $self, $condition_value );
    }
    ...
    public function section_addon_render( $self, $settings ) {
        if ( 'tab' == $settings['use_as'] ) {
			...
			<div class="<?php echo esc_attr( 'yes' == $settings['section_content_type'] ? 'elementor-container container-fluid' : 'elementor-container' ); ?> elementor-column-gap-no<?php echo esc_attr( $extra_class ); ?>">
				...
				<?php foreach ( $alpus_section['tab_data'] as $idx => $data ) { ?>
					<?php
                    $html      = '';
				    $icon_html             = '';
				    ...
				    if ( isset( $data['icon']['library'] ) && 'svg' === $data['icon']['library'] ) {
				        ...
				        $icon_html = '<div class="nav-icon">' . ob_get_clean() . '</div>';
				    } elseif ( $data['icon']['value'] ) {
				        $icon_html = '<div class="nav-icon"><i class="' . $data['icon']['value'] . '"></i></div>';
				    }
				    ...
				    if ( ! $data['icon'] && ! $data['title'] ) {
				        $html .= esc_html__( 'Tab Title', 'alpus-core' );
				    }
				    ?>
					<li class="nav-item<?php echo ! $data['icon']['value'] ? '' : ' nav-icon-' . esc_attr( $data['icon_pos'] ); ?>"><a class="nav-link<?php echo esc_attr( 0 === $idx ? ' active' : '' ); ?>" href="<?php echo esc_attr( $data['id'] ); ?>"><?php echo alpus_strip_script_tags( $html ); ?></a></li>
				<?php } ?>
				</ul>
    ...
    public function register_column_addon( $addons ) {
        $addons['tab_content'] = esc_html__( 'Tab Content', 'alpus-core' );
    ...
    public function add_column_controls( $self, $condition_value ) {
        ...
        $self->start_controls_section(
            'column_tab',
            array(
                'label'     => alpus_elementor_panel_heading( esc_html__( 'Tab Content', 'alpus-core' ) ),
                'tab'       => Controls_Manager::TAB_LAYOUT,
                'condition' => array(
        ...
        $self->add_control(
            ...
            array(
                ...
                'label'   => esc_html__( 'Nav Title', 'alpus-core' ),
                'default' => esc_html__( 'Nav Title', 'alpus-core' ),
            )
        );
        ...
        $self->add_control(
            'tab_icon',
            array(
                'label'       => esc_html__( 'Nav Icon', 'alpus-core' ),
                'description' => esc_html__( 'Choose icon for title of each tab item.', 'alpus-core' ),
        ...
        $self->add_control(
            'tab_icon_pos',
            array(
                'label'       => esc_html__( 'Nav Type With Icon', 'alpus-core' ),
                'description' => esc_html__( 'Choose icon position of each tab nav. Choose from Left, Up, Right, Bottom.', 'alpus-core' ),
                'default'     => 'left',
                'label_block' => 'true',
                'type'        => Controls_Manager::CHOOSE,
                'options'     => array(
                    'up'    => array(
                        'title' => esc_html__( 'Up', 'alpus-core' ),
                    ...
                    'down'  => array(
                        'title' => esc_html__( 'Down', 'alpus-core' ),
        ...
        $self->add_responsive_control(
            'tab_icon_space',
            array(
                'label'       => esc_html__( 'Nav Icon Spacing (px)', 'alpus-core' ),
                'description' => esc_html__( 'Controls spacing between icon and label in nav item.', 'alpus-core' ),
                ...
                'selectors'   => array(
                    '.tab .nav-link[href="{{ID}}"]' => '--alpus-tab-icon-space: {{SIZE}}px;',
                ),
            )
        );
        ...
        $self->add_responsive_control(
            'tab_icon_size',
            array(
                'label'       => esc_html__( 'Nav Icon Size (px)', 'alpus-core' ),
                'description' => esc_html__( 'Controls icon size of tab item header.', 'alpus-core' ),
                'type'        => Controls_Manager::SLIDER,
                'range'       => array(
                    'px' => array(
                        'step' => 1,
		...
		if( 'tab_content' == settings.use_as ) {
			...
			if ( settings.tab_icon && settings.tab_icon.value ) {
				if( settings.tab_icon.library && 'svg' == settings.tab_icon.library ) {
					...
					icon_html = '<div class=~' + "nav-icon" + '~>' + svgString.replaceAll('\"', '\~') + '</div>';
				} else {
					icon_html += '<div class=~' + "nav-icon" + '~><i class=~' + settings.tab_icon.value + '~></i></div>';
				}
			}
    ...
    public function column_addon_attributes( $options, $self, $settings ) {
        if ( 'tab_content' == $settings['use_as'] ) {
            ...
            if ( isset( $alpus_section['section'] ) ) {
                ...
                if ( 'tab' == $alpus_section['section'] && 0 == $alpus_section['index'] ) {
 ...
 * @since 1.0
 */
Alpus_Tab_Elementor_Widget_Addon::get_instance();
