...
if ( 'slider' == $layout_type ) {
    $wrapper_cls .= alpus_get_slider_class( $atts );
...
foreach ( $counters_list as $key => $item ) {
    ...
    if ( $item['title'] || $item['desc'] ) {
        ...
        if ( $item['title'] ) {
            ...
            $html .= '<' . $title_html_tag . ' ' . $this->get_render_attribute_string( $repeater_setting_key ) . '>' . alpus_strip_script_tags( $item['title'] ) . '</' . $title_html_tag . '>';
        }
        ...
        if ( $item['desc'] ) {
            ...
            $this->add_inline_editing_attributes( $repeater_setting_key );
            $html .= '<p ' . $this->get_render_attribute_string( $repeater_setting_key ) . '>' . alpus_strip_script_tags( $item['desc'] ) . '</p>';
        }
        $html .= '</div>';
    }
    $html .= '</div>';
